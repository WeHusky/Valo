...
// 2. Ambil data map saat ini dari database
$stmt = mysqli_prepare($conn, "SELECT * FROM maps WHERE id_map = ?");
mysqli_stmt_bind_param($stmt, "i", $map_id);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);
$map = mysqli_fetch_assoc($result);
mysqli_stmt_close($stmt);

// Jika map tidak ditemukan
if (!$map) {
    header('Location: /VALORANT/public/user/maps.php?error=' . urlencode('Map not found.'));
    exit;
}
?>
<!DOCTYPE html>
...
<body class="bg-[#0F1923] text-[#ECE8E1] min-h-screen">
    ...
    <main class="pt-32">
        <section class="max-w-2xl mx-auto p-8 bg-zinc-900/80 backdrop-blur-lg border border-zinc-700 rounded-lg shadow-2xl shadow-red-500/20">
            <h1 class="valorant-text text-4xl text-white text-center mb-8">EDIT MAP</h1>
            ...
            <form action="/VALORANT/app/edit_map_process.php" method="POST" enctype="multipart/form-data" class="space-y-6">
                
                <input type="hidden" name="id_map" value="<?= $map['id_map'] ?>">
                <input type="hidden" name="old_image_name" value="<?= $map['image_map'] ?>">
                
                <div>
                    <label for="name_map" class="block text-sm font-bold text-zinc-300 uppercase tracking-wider">Map Name</label>
                    <input type="text" id="name_map" name="name_map" required 
                           ...
                           value="<?= htmlspecialchars($map['name_map']) ?>">
                </div>
                <div>
                    <label for="desc_map" class="block text-sm font-bold text-zinc-300 uppercase tracking-wider">Description</label>
                    <textarea id="desc_map" name="desc_map" rows="4" required 
                              class="mt-1 block w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-red-500 transition-all"><?= htmlspecialchars($map['desc_map']) ?></textarea>
                </div>
                
                <div>
                    <label for="image_map" class="block text-sm font-bold text-zinc-300 uppercase tracking-wider">Map Image</label>
                    <p class="text-xs text-zinc-400 mt-1">Current: <?= htmlspecialchars($map['image_map']) ?>. Leave blank to keep.</p>
                    <input type="file" id="image_map" name="image_map" 
